...
description: Manages AgentOS AgentSpec YAML files under {drafts,published}/<slug>/vN.yaml — read, create, edit, clone, backup, publish, remove. List/read/write/validate/publish go through the AgenteOS MCP server (not direct filesystem access); validates every write against the two-layer schema (pydantic + JSON Schema). Use when the user says "/agentos-builder", "agentos-builder", "build an agent spec", "create a new agent", "edit an AgentSpec", "clone an agent spec", or wants to inspect/list existing agent YAML definitions.
license: MIT
metadata:
  author: w1
  version: "0.4.0"
...
Every list/read/write/validate/publish operation calls `scripts/mcp_client.py`, which reads `MCP_URL` and `MCP_KEY` from the environment and sends `POST {MCP_URL}/v1/tools/call` with `Authorization: Bearer <key>`. If either is missing, or the server rejects the key (`unauthorized`), the client raises and **the operation aborts** — surface the error to the user with the exact instruction to set `MCP_URL`/`MCP_KEY` (a key is issued via `mcp_server.auth.issue_key` for the sandbox tenant). **Never** fall back to reading or writing the YAML directly with Read/Write/Edit when the MCP call fails — a fallback here would silently mask real MCP gaps and defeat the entire point of proving the core/sandbox boundary (024/T018). If the MCP is down or misconfigured, the operation is simply unavailable right now; say so plainly.
...
- Running `make seed-catalog` (only `make seed-catalog-dry-run` is ever suggested, never run automatically as a real seed).
...
3. **Discovery, not memory.** Never hardcode or recall from a previous run which node types, agents, or schema fields exist. Always call `mcp_client.node_types()` fresh, every time — it returns the node-type catalog (with `runtime_ready`) plus the `trigger`/`spec_level` shape, sourced server-side from the builder map + the executor registry (T027, closes the gap this skill used to fill with direct core-repo reads — see `dogfood.md`).
4. **After any successful write**, suggest running `make seed-catalog-dry-run` to let the user validate the change against the catalog seed locally. Never run `make seed-catalog` (the real seed) yourself.
